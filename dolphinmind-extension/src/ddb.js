...
export async function ddbExecute(script) {
  console.log(`[DDB] ddbExecute called, connected=${ddbConnected}, ddb=${!!ddb}, script_len=${script?.length}`);
  if (!ddb || !ddbConnected) {
    console.error(`[DDB] ddbExecute: not connected! ddb=${!!ddb}, ddbConnected=${ddbConnected}`);
